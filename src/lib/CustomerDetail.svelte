<script lang="ts">
	...
	$: allServicesTableData = {
		...
		meta: tableMapperValues($allServices, [
			'serviceId',
			'paid',
			'pickedUp',
			'pickUpDate',
	...
	$: activeServicesTableData = {
		...
		meta: tableMapperValues($activeServices, [
			'serviceId',
			'paid',
			'pickedUp',
			'pickUpDate',
	...
	async function handleEditCustomer() {
		...
		const settings: ModalSettings = {
			type: 'component',
			component: s,
			title: `${customerData.customerInfo.label}`,
	...
	async function handleAddNewService(meta: unknown) {
		...
		const settings: ModalSettings = {
			type: 'component',
			component: s,
			title: `New Service for: ${customerData.customerInfo.label}`,
			body: `Customer Notes: ${customerData.customerInfo.notes}`,
			meta: customerData,
			buttonTextCancel: 'Back',
			response: (r) => console.log('response:', r)
...
{#if $modalStore[0]}
	<div class="modal-example-form {cBase}">
		...
		<div class="max-h-96 overflow-y-scroll">
			<Table
				regionHead={tableHeader}
				source={tableData}
				interactive={true}
				on:selected={serviceCheckInHandler}
			/>
		</div>
		<!-- prettier-ignore -->
